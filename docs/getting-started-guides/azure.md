...
The credential is saved in $HOME/.kube/config. We use username and password for authentication.
You should able to ssh to the VMs by ssh -p 20000 ${$AZ_CS}.cloudapp.net and
ssh -p 2000{1..4} ${$AZ_CS}.cloudapp.net

Access the endpoint in https://${KUBE_MASTER_IP}:443 by username and password in $HOME/.kube/config
...
Since you have kubeconfig in your $HOME/.kube/config, you should able to use kubectl to access
your cluster without any difficulty.(Such as kubectl get nodes)
